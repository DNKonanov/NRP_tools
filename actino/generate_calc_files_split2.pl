...
my %restart = ();
open my $rfh, '<', 'restartlist.txt' or die $!;
while(<$rfh>){
    chomp;
    my($l, $f) = split(/\t/, $_);
    $restart{$f} = $l;
...
my $mfh = undef;
my %done = ();
#if(-e "seqsim_matrix_re_$first.txt"){
#    die "ERROR $first seqsim exists\n";
...
#}else{
open $mfh, '>', "seqsim_matrix_remainder.txt" or die $!;
#}
my %sn = ();
foreach my $ss (glob("seq*txt")){
    open my $chk, '<', $ss or die $!;
    while(<$chk>){
	chomp;
	my @ln = split(/\|/, $_);
	$sn{$ln[3]} = 1;

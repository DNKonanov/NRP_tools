...
import sys
import os
## REMOVED BY MC: #from antismash import utils
from os import path ## ADDED BY MC

## ADDED BY MC from utils.py (antismash)
...
def run_nrpscodepred(infile2):
    #Extract 10 / 34 AA NRPS signatures from A domains
    ## REMOVED BY MC:  #infile = utils.get_full_path(__file__, "A_domains_muscle.fasta")
    infile = get_full_path(__file__, "A_domains_muscle.fasta") ## ADDED BY MC
    ## REMOVED BY MC:  #infile2 = "nrpsseqs.fasta"
    out_file1 = "input.sig"
    ## REMOVED BY MC:  #out_file2 = "ctg" + str(options.record_idx) + "_nrpspredictor2_codes.txt"
    out_file2 = "nrpspredictor2_codes.txt" ## ADDED BY MC
    ...
    for i in namesb:
      ...
      ## REMOVED BY MC:  #file = open(utils.get_full_path(__file__, "A34positions.txt"), "r")
      file = open(get_full_path(__file__, "A34positions.txt"), "r") ## ADDED BY MC
      text = file.read()
      text = text.strip()
      text = text.replace(' ','_')
      ...
      ## REMOVED BY MC:  #file = open(utils.get_full_path(__file__, "Apositions.txt"), "r")
      file = open(get_full_path(__file__, "Apositions.txt"), "r") ## ADDED BY MC
      text = file.read()
      text = text.strip()
      text = text.replace(' ','_')
    ...
    ## REMOVED BY MC: #infile3 = utils.get_full_path(__file__, "knowncodes.fasta")
    infile3 = get_full_path(__file__, "knowncodes.fasta") ## ADDED BY MC
